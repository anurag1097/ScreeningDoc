class FetchResult:
    ...
    def apply_lung_cancer_logic(self):
        ...
        if screening_interval and (self.number_of_pack_years >= 20 and 50 <= self.age <= 80):
            msg = "According to the USPSTF guidelines, you are eligible for lung cancer screening. This does not mean " \
    ...
    def apply_colon_cancer_logic(self):
        if not self.any_disease_history:
            if 85 >= self.age >= 76:
                msg = "Kindly discuss with your primary healthcare physician. As per the American Cancer Society, " \
            ...
            if self.params["familyHistoryCancer"] == "yes":
                string = "Some people with a family " \
            ...
            if self.params["personalHistoryCancer"] == "yes":
                string = "Most of these people will need to get a colonoscopy again after 3 years, " \
            ...
            if self.params["radiationHistory"] == "yes":
                string = "Most of these people will need to start having colorectal screening (colonoscopy " \
            ...
            if self.params["personalHistoryIBD"] == "yes":
                string = "DISCUSS THE FOLLOW-UP COLONOSCOPY WITH YOUR HEALTH CARE PROVIDER. \nAS PER THE AMERICAN " \
            ...
            if self.params["hereditaryColorectalCancer"]:
                string = "AS PER THE AMERICAN CANCER SCREENING GUIDELINES, INDIVIDUALS WITH CERTAIN GENETIC SYNDROMES " \
    ...
    def apply_breast_cancer_logic(self):
        if self.gender == 'female' and 50 <= self.age <= 75:
            msg = "Recommendation: According to the USPSTF guidelines, you are eligible for breast cancer " \
